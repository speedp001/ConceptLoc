...
# 여기서는 임베딩이나 feature는 보지 않고, 오직 3D 위치를 고려한다.
# 충분히 겹치는 마스크들을 하나로 통합한다.
# 후에 마스크들이 깔끔하게 병합되면 해당 좌표에 해당하는 임베딩 값을 full pcd에서 평균내서 넣어준다.
...
def seq_merge(frames_pcd, th, down_size, proxy_th):
    ...
    # apply one more merge
    # 전체적으로 겹치는데 시퀀스 별로 처리하다보니 중간에 끊겨서 병합이 안되는 경우를 고려하여 전체 mask를 한꺼번에 중복 병합 처리
