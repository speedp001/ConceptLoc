...
class Graph:
    ...
    def __init__(self, cfg: DictConfig):
        ...
        # 최종 3D 객체 마스크 특징
        self.mask_feats = []
        self.mask_feats_d = []  # 실제로 사용X
        
        # 최종 3D 객체 포인트 클라우드
        self.mask_pcds = []
        self.mask_weights = []  # 실제로 사용X
        ...
        # create_graph.yaml에 pipline 설정이 없으면 평가 모드
    ...
    def create_feature_map(self, save_path=None):
        ...
        for i in tqdm(range(0, len(self.dataset), self.cfg.pipeline.skip_frames), desc="Creating RGB-D point cloud"):
            ...
            # 3차원 공간에 포인트 클라우스 생성
            self.full_pcd += self.dataset.create__pcd(rgb_image, depth_image, pose)

        # filter point cloud
        # voxel_down_sample: open3d 내부 함수 -> 포인트 클라우드 다운샘플링
        ...
        # DBScan 필터링
        # 이웃 포인트 거리 기반 노이즈 제거
        # 반경 eps 안에 최소 min_points 개수 이상 이웃이 있어야 클러스터 일부로 인정
        ...
        # 각 full pcd 점이 몇 번의 프레임에서 feature를 받았는지 개수 연산
        # 해당 점에 대해 누적한 feature 개수
        counter = torch.zeros((n_points, 1), device="cpu")
        # 각 full pcd 점에 대해 누적한 feature 합계
        sum_features = torch.zeros((n_points, self.clip_feat_dim), device="cpu")

        # extract features for each frame
        # 프레임 수 -> 해당 프레임에서 얻은 3D 마스크 리스트
        frames_pcd = []
        # 프레임 수 -> 해당 프레임에서 얻은 특징 리스트
        frames_feats = []
        ...
        for i in tqdm(range(0, len(self.dataset), self.cfg.pipeline.skip_frames), desc="Extracting features"):
            ...
            """
            F_2D의 형태 -> 각 이미지에서 유효한 각 픽셀에 해당하는 CLIP 임베딩 가중합
            F_2D = torch.tensor([
                [0.12, -0.45, 0.78, ..., 0.23],  # 픽셀 (0,0)의 CLIP feature (768-dim)
                [0.34, -0.12, 0.56, ..., -0.11], # 픽셀 (0,1)
                [0.22, 0.67, -0.34, ..., 0.45],  # 픽셀 (0,2)
                ...
                [0.11, 0.33, 0.22, ..., -0.66],  # 마지막 유효 픽셀
            ])
            """
            """
            masks의 형태 -> 각 이미지에서 추출된 SAM 마스크 리스트
            masks = [
                {
                    "segmentation": np.array([[False, False, True, ...],   # (480, 640) boolean
                                            [False, True, True, ...],
                                            ...]),
                    "bbox": [x_min, y_min, width, height],  # [100, 200, 50, 80]
                    "area": 4000,
                    "predicted_iou": 0.95,
                },
                {
                    "segmentation": np.array([[True, False, False, ...],   # 두 번째 객체
                                            [True, True, False, ...],
                                            ...]),
                    "bbox": [250, 150, 60, 90],
                    "area": 5400,
            ...
            """
            """
            F_masks의 형태 -> 각 이미지에서 추출된 SAM 마스크별 CLIP 임베딩
            F_masks = torch.tensor([
                [0.15, -0.22, 0.88, ..., 0.33],  # mask 0 (의자)의 평균 feature
                [0.44, 0.12, -0.56, ..., 0.11],  # mask 1 (테이블)의 평균 feature
                [0.32, 0.77, 0.24, ..., -0.45],  # mask 2 (램프)의 평균 feature
                ...
                [0.21, 0.43, 0.12, ..., 0.76],   # mask 14의 평균 feature
            ])
            """
            ...
            """
            masks_3d의 형태 -> 각 이미지에서 추출된 SAM 마스크별 3D 포인트 클라우드 리스트
            [
                o3d.geometry.PointCloud of mask 0 (의자),
                o3d.geometry.PointCloud of mask 1 (테이블),
                o3d.geometry.PointCloud of mask 2 (램프),
                ...
                o3d.geometry.PointCloud of mask 14,
            ]
            """
        ...
        # 여기서 생성된 각 마스크 좌표에 full pcd에서 임베딩 값을 할당
        masks_feats = []
    ...
    def segment_floors(self, path, flip_zy=False):
        ...
        # 데이터셋마다 바닥이 y축인지 z축인지 다르기 때문에 바닥이 y축이 되도록 변환
        ...
        reselotion = 0.01
        # y축을 따라 1cm 간격으로 포인트 수를 세는 히스토그램 생성
        ...
        # 3단계 스무딩
        # [0.2, 1.5, 4.0, 8.0, 15.0, ..., 80.0, 70.0, 60.0, ...]
        ...
        # 4단계 상위 10% peak 추출
        # peaks -> [ 30, 310, 330, 610, 630, 900 ]
        # peaks location -> [ 0.30, 3.10, 3.30, 6.10, 6.30, 9.00 ]

        # 5단계 클러스터링
        # [0.30]        -> cluster 0 -> 제일 바닥은 top1만 뽑아서 하나의 면만 추출
        # [3.10, 3.30]  -> cluster 1 -> 1층 천장과 2층 바닥
        # [6.10, 6.30]  -> cluster 2 -> 2층 천장과 3층 천장
        # [9.00]        -> cluster 3 -> 마지막 천장은 top1만 뽑아서 하나의 면만 추출
        
        # 층 나누기 핵심 알고리즘
        # 중간층은 면이 두개이므로 2개의 peak를 선택
        # for each cluster find the top 2 peaks
        clustred_peaks = []
        for i in range(len(np.unique(labels))):
            # 처음과 마지막은 면이 하나이므로 1개의 peak 선택
        ...
        # 6단계 평탄화
        # 클러스터링 된 peak들을 1차원으로 변환 [0.2, 2.8, 3.1, 5.0, 5.3, 7.2, 7.4, 9.3, 9.5, 11.0]
        
        # 층수는 K-1 (클러스터링 된 개수에서 - 1 = 층수)
        floors = []
    ...
    def segment_rooms(self, floor: Floor, path):
        ...
        # 천장과 바닥 근처를 제거 -> 바닥에서 1.5m 위, 천장에서는 30cm 아래
        ...
        # 평면을 격자로 나눠서 각 칸에 해당하는 포인트 빈도 히스토그램 생성
        ...
        for i in tqdm(range(len(room_vertices)), desc="Assign floor points to rooms"):
            ...
            # room_vertices가 각 방의 (x, y) 좌표 리스트
            # mask는 해당 좌표에 255 값을 할당 -> 색칠하기
            room[room_vertices[i][0], room_vertices[i][1]] = 255
            room_masks.append(room)
            # room 인덱스 좌표를 point cloud 좌표로 변환
            ...
            T1[:3, :3] = Rotation.from_euler("x", 90, degrees=True).as_matrix()
            pcd.transform(T1)
            # 3차원 인덱스 번호로 실제 point cloud에서 KDTree로 실제 좌표 탐색
        ...
        # 여기서 저장하는건 skip_frames 단위로 뽑은 이미지의 CLIP 전역 특징
        np.savez(
        ...
        # 카메라 pose를 R|t로 계산하여 room_pcd 범위 내 인지 판별
