...
def extract_feats_per_pixel(
    ...
    # CLIP feature fusion -> Bounding box & Segmentation 가중합
    ...
    """
    컨텍스트와 잘 맞는 마스크일수록 글로벌 의미 F_g 비중을 많이 섞어주고,
    덜 맞는 마스크는 거의 로컬 정보 F_l[i] 그대로 두는 식으로,
    “장면 전체 의미를 반영한 마스크 임베딩”을 만든다.
    """
    ...
    # 마스크의 임베딩 값을 뽑았으면 해당 마스크 픽셀이 해당하는 곳에 임베딩 값 할당
    # 따라서 공통 픽셀이 여러 마스크에 들어있을 경우 이를 Normalize 해서 합산 -> Scale이 일정해야하기 때문
    F_p = F_p.cuda()
